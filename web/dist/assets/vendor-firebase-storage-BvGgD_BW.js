import{_ as te,r as S,S as ne,a as se,g as re,b as oe}from"./vendor-firebase-app-CZHFuJSY.js";import{F as ie,c as P,A as ae,h as B,p as ue,u as ce,B as le}from"./vendor-firebase-util-DLzrvjrQ.js";import{C as he}from"./vendor-firebase-component-BsAoxdSC.js";/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class f{constructor(t,n){this.bucket=t,this.path_=n}get path(){return this.path_}get isRoot(){return this.path.length===0}fullServerUrl(){const t=encodeURIComponent;return"/b/"+t(this.bucket)+"/o/"+t(this.path)}bucketOnlyServerUrl(){return"/b/"+encodeURIComponent(this.bucket)+"/o"}static makeFromBucketSpec(t,n){let s;try{s=f.makeFromUrl(t,n)}catch{return new f(t,"")}if(s.path==="")return s;throw we(t)}static makeFromUrl(t,n){let s=null;const r="([A-Za-z0-9.\\-_]+)";function o(g){g.path.charAt(g.path.length-1)==="/"&&(g.path_=g.path_.slice(0,-1))}const i="(/(.*))?$",a=new RegExp("^gs://"+r+i,"i"),u={bucket:1,path:3};function h(g){g.path_=decodeURIComponent(g.path)}const d="v[A-Za-z0-9_]+",R=n.replace(/[.]/g,"\\."),m="(/([^?#]*).*)?$",k=new RegExp(`^https?://${R}/${d}/b/${r}/o${m}`,"i"),T={bucket:1,path:3},E=n===j?"(?:storage.googleapis.com|storage.cloud.google.com)":n,_="([^?#]*)",A=new RegExp(`^https?://${E}/${r}/${_}`,"i"),O=[{regex:a,indices:u,postModify:o},{regex:k,indices:T,postModify:h},{regex:A,indices:{bucket:1,path:2},postModify:h}];for(let g=0;g<O.length;g++){const U=O[g],v=U.regex.exec(t);if(v){const ee=v[U.indices.bucket];let x=v[U.indices.path];x||(x=""),s=new f(ee,x),U.postModify(s);break}}if(s==null)throw be(t);return s}}class Ae{constructor(t){this.promise_=Promise.reject(t)}getPromise(){return this.promise_}cancel(t=!1){}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function Ue(e,t,n){let s=1,r=null,o=null,i=!1,a=0;function u(){return a===2}let h=!1;function d(..._){h||(h=!0,t.apply(null,_))}function R(_){r=setTimeout(()=>{r=null,e(k,u())},_)}function m(){o&&clearTimeout(o)}function k(_,...A){if(h){m();return}if(_){m(),d.call(null,_,...A);return}if(u()||i){m(),d.call(null,_,...A);return}s<64&&(s*=2);let O;a===1?(a=2,O=0):O=(s+Math.random())*1e3,R(O)}let T=!1;function E(_){T||(T=!0,m(),!h&&(r!==null?(_||(a=2),clearTimeout(r),R(0)):_||(a=1)))}return R(0),o=setTimeout(()=>{i=!0,E(!0)},n),E}function Ne(e){e(!1)}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
