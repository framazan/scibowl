...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    // Resolve background color: supports 'transparent', CSS color strings, or [r,g,b,a] in 0..1
    function resolveBgVec4(bg) {
      if (bg === 'transparent' || bg == null) return new THREE.Vector4(0, 0, 0, 0);
      if (Array.isArray(bg) && bg.length === 4) {
        const [r, g, b, a] = bg;
        return new THREE.Vector4(r ?? 0, g ?? 0, b ?? 0, a ?? 1);
      }
      try {
    ...
    const bgVec4 = resolveBgVec4(backgroundColor);
    ...
    class MouseClass {
      ...
      init(container) {
        this.container = container;
        // Use document-level listeners so interaction works even when content overlays the canvas
